...
class ResultsViewer:
    ...
    def plot3(self,
            ...
            result1_label: str = "Result 1",
            result2_label: str = "Result 2",
            result1_color: str = "tab:blue",
            result2_color: str = "tab:red"
            ):
        """
        Plot results with a beautiful gene track (exons matched by coordinate containment).
        result1_label / result2_label: 自定义第一个/第二个结果的名字（图例与子图标签）。
        传 None 或空字符串时回退到默认 "Result 1"/"Result 2"。
        """
        values = results.get("values", {})
        position = results.get("position", (None, None, None))
        chrom, start, end = position if len(position) == 3 else (None, None, None)
        ...
        if not use_transcripts:
            ...
            for tx in transcripts_f:
                ...
                # 方向小三角（方案B：内含子线中点）：+链朝右 >，-链朝左 <
                mid_x = (ts + te) / 2
                marker = '>' if tstrand == "+" else '<'
                ax_gene.scatter([mid_x], [y], marker=marker, s=70, color=color,
        ...
        for head in track_names:
            ...
            for b in biosamples:
                ...
                if arr is not None:
                    ...
                    if smoothing_sigma > 0:
                        y = gaussian_smooth(y, smoothing_sigma)
                    ax.plot(display_positions, y, color=result1_color, linewidth=1.5, alpha=0.9, label=(result1_label or "Result 1") if b == biosamples[0] else "")
                ...
                if results2:
                    ...
                    if arr2 is not None:
                        ...
                        if smoothing_sigma > 0:
                            y2 = gaussian_smooth(y2, smoothing_sigma)
                        ax.plot(display_positions, y2, color=result2_color, linewidth=1.5, alpha=0.9, linestyle='--', label=(result2_label or "Result 2") if b == biosamples[0] else "")
        ...
        if show_legend and biosamples:
            handles = []
            labels = []
            handles.append(Line2D([0], [0], color=result1_color, lw=2, linestyle='-'))
            labels.append(result1_label or "Result 1")
            if results2:
                handles.append(Line2D([0], [0], color=result2_color, lw=2, linestyle='--'))
                labels.append(result2_label or "Result 2")
            # 过滤排除的 biosample
            for i, b in enumerate(biosamples):
