...
def _parse_structured_dataset_entries(
    ...
    for i, entry in enumerate(entries):
        ...
        # Optional region mask track (0/1 BigWig). May be specified per-entry or globally.
        # Accept a few aliases to make configs less brittle.
        region = None
        for k in ("region_path", "mask_path", "region_bw_path", "region_bw", "region_file"):
            v = entry.get(k)
        ...
        if region:
            region = str(region)
            _require_file(region, context=f"dataset entry {name!r} (region_path)")
            region_files[name] = region
    ...
    return (
        ...
        default_fasta_str,
        per_cell_fasta,
        region_files if region_files else None,
    )


def effective_per_device_train_batch_size(cfg: dict[str, Any]) -> int:
    """``training.per_device_train_batch_size`` if set, else top-level ``batch_size``, else 3."""
...
def effective_inference_batch_size(cfg: dict[str, Any]) -> int:
    """``inference_batch_size`` if set, else top-level ``batch_size``, else 1."""
