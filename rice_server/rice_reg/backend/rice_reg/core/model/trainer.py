...
from model.distributed import DistributedSamplerCallback, is_main_process
...
class SaveEveryNEpochCallback(TrainerCallback):
    """Gate epoch-based checkpoint saving to every N epochs.

    Intended to be used with `TrainingArguments(save_strategy="epoch")`.
    """

    def __init__(self, n: int):
        self.n = int(n)
        if self.n < 1:
            raise ValueError("SaveEveryNEpochCallback n must be >= 1")

    def on_epoch_end(self, args, state, control, **kwargs):
        if self.n == 1:
            return control
        # HF uses float epochs; at epoch end it should be close to an integer.
        try:
        ...
        if epoch_i < 1:
            return control
        control.should_save = (epoch_i % self.n == 0)
        return control
...
class CustomTrainer(Trainer):
    """Default (v0 / MSE head): multimodal forward, DDP dataloader, CSV loss log, safetensors save."""
    ...
    def _extend_csv_row_with_aux_losses(self, row: dict, logs: dict) -> None:
        """H2 adds cls_loss / reg_loss; v0 leaves them out."""
        pass
...
class CustomTrainerH2(CustomTrainer):
    """H2 head: running cls_loss / reg_loss sums and same-interval averaging as HF `loss`."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._h2_cls_sum = None
        self._h2_reg_sum = None
        # HF updates _globalstep_last_logged before calling log(), so log() cannot infer the
        # step interval from global_step - _globalstep_last_logged (always 0). Stash the true
        # interval here in the same way HF uses it for averaging `loss`.
        self._h2_log_interval = None
    ...
    def _maybe_log_save_evaluate(
        ...
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            self._h2_log_interval = self.state.global_step - self._globalstep_last_logged
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        loss = outputs["loss"]
        if outputs.get("cls_loss") is not None:
            if self._h2_cls_sum is None:
                self._h2_cls_sum = torch.tensor(
                    0.0, device=self.args.device, dtype=torch.float32
                )
                self._h2_reg_sum = torch.tensor(
                    0.0, device=self.args.device, dtype=torch.float32
                )
            self._h2_cls_sum = self._h2_cls_sum + outputs["cls_loss"].detach().float()
            self._h2_reg_sum = self._h2_reg_sum + outputs["reg_loss"].detach().float()
        return (loss, outputs) if return_outputs else loss

    def log(self, logs, start_time=None):
        if "loss" in logs and self._h2_cls_sum is not None:
            interval = self._h2_log_interval
            if interval is not None and interval > 0:
                cls_sum = self._nested_gather(self._h2_cls_sum).mean()
                reg_sum = self._nested_gather(self._h2_reg_sum).mean()
                denom = float(interval) * float(getattr(self.args, "gradient_accumulation_steps", 1) or 1)
                logs["cls_loss"] = round((cls_sum / denom).item(), 6)
                logs["reg_loss"] = round((reg_sum / denom).item(), 6)
                self._h2_cls_sum.zero_()
                self._h2_reg_sum.zero_()
        self._h2_log_interval = None
        super().log(logs, start_time)

    def _extend_csv_row_with_aux_losses(self, row: dict, logs: dict) -> None:
        if "cls_loss" in logs:
            row["cls_loss"] = logs["cls_loss"]
        if "reg_loss" in logs:
            row["reg_loss"] = logs["reg_loss"]
...
def _split_trainable_params_discriminative_lr(model: torch.nn.Module) -> tuple[list[torch.nn.Parameter], list[torch.nn.Parameter]]:
    ...
    (see predictors like `model/predictor_v0.py`). For discriminative LR, we treat that last
...
class CustomTrainerDiscriminativeLRH2(CustomTrainerDiscriminativeLR, CustomTrainerH2):
    """H2 variant of discriminative LR trainer (keeps H2 logging)."""

    # MRO ensures __init__/create_optimizer come from CustomTrainerDiscriminativeLR,
    # while compute_loss/log behavior comes from CustomTrainerH2.
    pass
