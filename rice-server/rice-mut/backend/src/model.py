...
def predictions_scaling_torch(
    ...
    # inverse squashing: first undo the soft-clip, then undo the power transform
    inv_pow = 1.0 / 0.75
    preds_quad = torch.where(
        preds > 10.0,
        (preds + 10.0).pow(2) / (4.0 * 10.0),
        preds
    )
    preds_pow = preds_quad.pow(inv_pow)
