...
from model.predictor_h2 import MultiModalPredictorH2
from model.predictor_h2b import MultiModalPredictorH2B
from model.predictor_bi import MultiModalPredictorBI
from model.predictor_v0 import MultiModalPredictor
from model.predictor_wtd import MultiModalPredictor as MultiModalPredictorWtd
from model.predictor_ss import MultiModalPredictorSS
from model.predictor_ss2 import MultiModalPredictorSS2
from model.predictor_block import MultiModalPredictorBlock
...
from model.trainer import (
    ...
    CustomTrainerDiscriminativeLRH2,
...
def _predictor_type_raw(cfg: dict):
    """Read raw predictor type without `or`-chaining (explicit keys only; falsy values like 0 are kept)."""
    block = _predictor_config_block(cfg)
    for key in ("type", "predictor_type"):
        if key in block:
            v = block[key]
            if v is not None:
                return v
    if "predictor_type" in cfg:
        v = cfg["predictor_type"]
        if v is not None:
            return v
    return "v0"
...
def _normalize_predictor_type(cfg: dict) -> str:
    """Return `v0` or `h2` (lowercased) from config; default `v0` if unset.

    Reads `predictor.type` / `predictor.predictor_type`, or legacy top-level `predictor_type`.
    """
...
def _resolve_dataset_type(cfg: dict) -> str:
    """Return ``v0`` (paired two-channel labels per window) or ``strand`` (strand-split samples).

    YAML key: ``dataset_type`` (default ``v0``).
    """
    raw = cfg.get("dataset_type", "v0")
    s = str(raw).strip().lower()
    if s not in ("v0", "strand"):
        raise ValueError(
            f"dataset_type must be 'v0' or 'strand', got {raw!r}"
        )
    return s


def _validate_dataset_type_match(dataset_type: str, ptype: str, model_cls: type) -> None:
    """Ensure ``dataset_type`` matches the predictor's ``REQUIRED_DATASET_TYPE`` if set.

    Predictors without ``REQUIRED_DATASET_TYPE`` default to ``v0`` (paired window dataset).
    """
...
def _h2_kwargs(cfg: dict) -> dict:
    """H2-only hyperparameters: prefer `predictor:` block, fall back to legacy `training:` keys."""
    block = _predictor_config_block(cfg)
    tcfg = cfg.get("training", {}) or {}
    return {
        "lambda_reg": float(
            block.get("lambda_reg", tcfg.get("lambda_reg", 1.0))
        ),
        "classification_pos_weight": float(
            block.get(
                "classification_pos_weight",
                tcfg.get("classification_pos_weight", 10.0),
            )
        ),
    }
...
def build_multimodal_model(cfg: dict, base_model):
    """Construct ATAC-conditioned predictor from YAML (v0, wtd, h2, h2b, ss, ss2). Used by training and inference."""
    ptype = _normalize_predictor_type(cfg)
    if ptype == "v0":
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        model = MultiModalPredictor(base_model, atac_encoder)
    elif ptype == "bi":
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        model = MultiModalPredictorBI(base_model, atac_encoder)
    elif ptype == "wtd":
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        model = MultiModalPredictorWtd(base_model, atac_encoder)
    elif ptype == "h2":
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        h2 = _h2_kwargs(cfg)
        model = MultiModalPredictorH2(
            base_model,
            atac_encoder,
            lambda_reg=h2["lambda_reg"],
            classification_pos_weight=h2["classification_pos_weight"],
        )
    elif ptype == "h2b":
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        h2 = _h2_kwargs(cfg)
        model = MultiModalPredictorH2B(
            base_model,
            atac_encoder,
            lambda_reg=h2["lambda_reg"],
            classification_pos_weight=h2["classification_pos_weight"],
        ...
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        model = MultiModalPredictorSS2(base_model, atac_encoder)
    elif ptype == "block":
        atac_encoder = ATAC_Encoder(output_dim=cfg.get("atac_encoder_output_dim", 1024))
        model = MultiModalPredictorBlock(base_model, atac_encoder)
    elif ptype == "fusion":
        ...
        raise ValueError(
            f"Unknown predictor type {ptype!r}. Use 'v0', 'bi', 'wtd', 'h2', 'h2b', 'ss', 'ss2', 'block', or 'fusion' "
            "(under predictor.type or legacy predictor_type)."
        )
    return model


def _resolve_output_training_dir(cfg: dict) -> str:
    """Directory for checkpoints, logs, and train_index CSV. Prefer ``output_training_dir``."""
    out = cfg.get("output_training_dir") or cfg.get("output_base_dir")
    if not out:
        raise ValueError(
            "Config must set output_training_dir (canonical for training). "
            "Legacy output_base_dir is still accepted."
        )
...
def run_training(cfg: dict):
    ...
    try:
        ...
        dist_print(f">>> dataset_type: {dataset_type}")
        ...
        if ptype == "block":
            from model.dataset_block import LazyGenomicDatasetBlock as _LazyGenomicDataset
        elif dataset_type == "strand":
            from model.dataset_strand import LazyGenomicDataset as _LazyGenomicDataset
        else:
            from model.dataset import LazyGenomicDataset as _LazyGenomicDataset

        n_rows = len(train_index_df)
        dist_print(
            f"\n>>> Creating dataset (index rows: {n_rows}, dataset_type={dataset_type})"
        )
        ...
        if ptype == "block":
            from model.trainer_block import CustomTrainerBlock
            trainer_cls = CustomTrainerBlock
        elif use_discriminative_lr:
            if ptype in ("h2", "h2b"):
                trainer_cls = CustomTrainerDiscriminativeLRH2
            elif ptype == "fusion":
                trainer_cls = CustomTrainerDiscriminativeLRFusion
            else:
                trainer_cls = CustomTrainerDiscriminativeLR
        else:
            if ptype in ("h2", "h2b"):
                trainer_cls = CustomTrainerH2
            elif ptype == "fusion":
        ...
        if len(train_dataset) > 0:
            ...
            with torch.no_grad():
                if ptype == "block":
                    dist_print(f"  region_signal: {sample['region_signal'].shape}")
                    dist_print(f"  region_mask  : {sample['region_mask'].shape}")
                    out = model(
                        input_ids=sample["input_ids"].unsqueeze(0).to(device),
                        atac_signal=sample["atac_signal"].unsqueeze(0).to(device),
                        region_signal=sample["region_signal"].unsqueeze(0).to(device),
                        region_mask=sample["region_mask"].unsqueeze(0).to(device),
